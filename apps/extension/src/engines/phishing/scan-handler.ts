...
import { ThreatScorer, ThreatLevel, type DomSignalInput } from "./threat-scorer";
...
export function handleScan(request: ScanRequest): ScanVerdict {
    ...
    const contentPatterns = pageText
        ? signatureDb.matchContent(pageText)
        : [];
    ...
    const assessment = scorer.assess({
        ...
        signatureMatch: {
            ...
            contentPatterns: contentPatterns.map((p) => p.description ?? p.pattern ?? ""),
